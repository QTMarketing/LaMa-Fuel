-- Supabase Migration Script for LaMa Fuel
...
CREATE POLICY IF NOT EXISTS "Service role can manage form_submissions"
  ON public.form_submissions
  FOR ALL
  USING (auth.role() = 'service_role');

CREATE POLICY IF NOT EXISTS "Service role can manage reviews"
  ON public.reviews
  FOR ALL
  USING (auth.role() = 'service_role');

-- Refresh the schema cache so PostgREST recognizes the new tables
